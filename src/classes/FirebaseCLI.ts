...
export class FirebaseCLIOperator {
  ...
  public async deployHosts(hosts: string[], project: string): Promise<void> {
    await this.tiApi?.updateDeployment({ status: DeploymentStatus.DEPLOY_APP });

    try {
      for (let host of hosts) {
        this.notification.info(`Deploying ${host}`);
        ...
        this.notification.success(`host ${host} deployed`);
      }
    } catch (e) {
      this.notification.error('Error deploying hosting');
      throw e;
    }
  }
}
