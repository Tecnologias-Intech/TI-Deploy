...
const run = async (): Promise<void> => {
  ...
  const devProject = core.getInput('dev-project', { required: true });
  const prodProject = core.getInput('prod-project', { required: true });
  ...
  try {
    ...
    const hostsString = core.getInput('firebase-hosts', { required: true });
    ...
    await firebaseCLI.deployHosts(hosts, prodProject);
